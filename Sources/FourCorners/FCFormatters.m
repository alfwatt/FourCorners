...
+ (NSDictionary*) unitsAttrs:(NSDictionary*) attrs {
    ...
    ILColor* textColor = attrs[NSForegroundColorAttributeName]; // if it's the controlTextColor we expect, make it disabled, otherwise passthrough
...
- (NSAttributedString *)attributedStringForObjectValue:(id)anObject withDefaultAttributes:(NSDictionary *)attrs {
    ...
    if ([anObject isKindOfClass:FCLocation.class]) {
        ...
        [formatted appendAttributedString:[NSAS.alloc initWithString:NSLocalizedString(@"°\u00a0", @"") attributes:cardinalAttrs]];
        [formatted appendAttributedString:[NSAS.alloc initWithString:
        ...
        /*
         ...
         if ([anObject altitude] > 0) { // the formatter below won't include the '+' char, which we want in the unitsAttrs, when above MSL
...
- (NSAttributedString*) attributedStringForObjectValue:(id) anObject withDefaultAttributes:(NSDictionary*) attrs {
    ...
    if (bearing.intValue == -1) {
        ...
        [bearingString appendAttributedString:[NSAS.alloc initWithString:NSLocalizedString(@" ~ ", @"") attributes:cardinalAttrs]];
...
- (NSString*) stringForObjectValue:(id) object {
    ...
    if ([object isKindOfClass:NSNumber.class]) {
        ...
        if( bearing.intValue == -1) {
            ...
            string = [self.class bearingString:bearing.doubleValue];
        }
    }
